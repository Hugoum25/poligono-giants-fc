...
rodri_src = 'src/assets/players/17-rodrigo-cuesta/rodri.png'
rodri_photo = 'src/assets/players/17-rodrigo-cuesta/photo.png'

if os.path.exists(rodri_src):
    shutil.copy(rodri_src, rodri_photo)

with open(rodri_src, 'rb') as f:
    b64_str = 'data:image/png;base64,' + base64.b64encode(f.read()).decode('utf-8')

# Update players.json for Rodrigo Cuesta (ID 17)
with open('src/data/players.json', 'r', encoding='utf-8') as f:
    players = json.load(f)

for p in players:
    if p['id'] == 17 or 'rodrigo' in p['name'].lower() or 'cuesta' in p['name'].lower():
        p['photo'] = b64_str

with open('src/data/players.json', 'w', encoding='utf-8') as f:
    json.dump(players, f, ensure_ascii=False, indent=2)

# Update teamData.js for player ID 17
...
code = code.replace('fc_hub_players_posligua_v45', 'fc_hub_players_rodri_v50')

with open('src/data/teamData.js', 'w', encoding='utf-8') as f:
    f.write(code)

print('Rodrigo Cuesta photo successfully processed and linked in players.json and teamData.js!')
